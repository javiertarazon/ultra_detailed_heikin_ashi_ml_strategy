#!/usr/bin/env python3
"""
Script para ejecutar un backtest utilizando los datos recopilados en modo live.
...
Flujo:
1. Carga los archivos CSV de datos live
2. Consolida los datos en un DataFrame
3. Ejecuta el backtest con estos datos
4. Compara resultados con las señales que se habrían generado en vivo
"""
...
async def consolidate_live_data(symbol='BTC_USDT', timeframe='15m'):
    """
    ...
    for file in files:
        ...
        try:
            ...
            if not all(['timestamp', 'open', 'high', 'low', 'close', 'volume'] <= df.columns):
                logger.warning(f"Archivo {file} no tiene las columnas esperadas, omitiendo")
                continue
            
            # Manejo explícito para archivos con columna timestamp duplicada
            # Verificar estructura de columnas
            logger.debug(f"Columnas del archivo: {df.columns.tolist()}")
            
            # Verificar si el archivo tiene la estructura esperada y columna duplicada
            if len(df.columns) == 7 and 'timestamp.1' in df.columns:
                logger.debug(f"Detectada columna timestamp duplicada como 'timestamp.1' en {file}")
                # Seleccionar solo las primeras 6 columnas (ignorar la duplicada)
                df = df.iloc[:, 0:6]
                logger.debug(f"Columnas después de eliminar duplicados: {df.columns.tolist()}")
            
            # Asegurarnos de que el DataFrame tiene las 6 columnas necesarias
            if len(df.columns) != 6:
                logger.error(f"El archivo {file} tiene {len(df.columns)} columnas en lugar de 6")
                for i, col in enumerate(df.columns):
                    logger.debug(f"Columna {i}: {col}")
                raise ValueError(f"Número de columnas incorrecto: {len(df.columns)}")
            
            # Renombrar las columnas si es necesario para asegurar nombres consistentes
    ...
    # Eliminar duplicados
    consolidated_df = consolidated_df.drop_duplicates(subset='timestamp').reset_index(drop=True)
    ...
    start_date = consolidated_df['timestamp'].min().strftime('%Y-%m-%d')
    end_date = consolidated_df['timestamp'].max().strftime('%Y-%m-%d')
...
async def run_backtest_with_live_data():
    ...
    data = await consolidate_live_data(csv_symbol, timeframe)
    ...
    for strategy_name, strategy in strategies.items():
        ...
        try:
            ...
            if strategy_result:
                ...
                signals[strategy_name] = strategy_result.get('signals', [])
    ...
    output_file = output_dir / f"live_data_backtest_results.json"
...
if __name__ == "__main__":
    # Configurar logging
    setup_logging("DEBUG", log_file="../logs/backtest_live_data.log")
